    ...
    <div class="container-fluid py-4 px-4">
      ...
      <div class="d-flex align-items-center gap-3 mb-4 flex-wrap">
        ...
        <div class="flex-grow-1">
          <h3 class="mb-0"><i class="bi bi-receipt me-2 text-primary"></i>Detalle de Factura</h3>
      ...
      <div *ngIf="!loading() && data()">
          ...
          <div class="card-body">
            <div class="row g-3">
              ...
              <div class="col-md-1 text-end">
                ...
                <div class="fw-bold text-primary">\${{ data().documento?.valor_neto | number:'1.0-0' }}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div *ngIf="!data().lineas?.length" class="text-center py-5 text-muted">
          ...
          <p class="mt-2">Esta factura no tiene líneas</p>
        </div>
      </div>
    </div>
  `
...
export class DetalleFacturaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (rowid) {
      ...
      this.http.get<any>(`http://localhost:3000/api/ventas/facturas/${rowid}`).subscribe({
  ...
  volver() {
    this.router.navigate(['/ventas'], { queryParams: { tab: 'facturas' } });
  }
  ...
  exportarCsv() {
    ...
    const headers = ['Referencia', 'Producto', 'Bodega', 'Sucursal', 'Motivo', 'Desc. Motivo', 'Unidad', 'Cantidad', 'Precio', 'Valor Bruto', 'Total'];
    const rows = lineas.map(l => [l.referencia, l.producto, l.bodega, l.sucursal, l.motivo, l.motivo_descripcion, l.unidad, l.cantidad, l.precio, l.valor_bruto, l.valor_total]);
    const csv = [headers, ...rows].map(r => r.map(c => `"${String(c ?? '').replace(/"/g, '""')}"`).join(',')).join('\n');
    const blob = new Blob(['\ufeff' + csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `factura_${doc?.consecutivo || 'detalle'}.csv`;
    a.click();
